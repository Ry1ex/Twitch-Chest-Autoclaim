...
class GestionnaireDePointsDeChaine {
    ...
    cliquerSurCoffre() {
        ...
        if (coffres.length > 0) {
            for (let coffre of coffres) {
                ...
                // Ajoute les points des coffres au total
    ...
    mettreAJourAffichageStats() {
        ...
        if (statsDiv) {
            ...
            statsDiv.innerHTML = `
                <p>Vous regardez le stream depuis <span>${tempsEcoule}</span>.</p>
    ...
    simulerPresenceUtilisateur() {
        ...
        window.addEventListener('blur', () => {
            setTimeout(() => {
                window.focus();
                console.log(this.tempsString() + " [CPA Custom Stats] Forçage du focus sur l'onglet.");
            }, 1000); // Attendre un peu avant de refocaliser
        });
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                setTimeout(() => {
                    window.focus();
                    console.log(this.tempsString() + " [CPA Custom Stats] Forçage du focus lorsque l'onglet devient invisible.");
                }, 1000);
            } else {
                console.log(this.tempsString() + " [CPA Custom Stats] L'onglet est de nouveau visible.");
            }
        });
